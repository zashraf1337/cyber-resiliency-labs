...
_The newsletter edition. By **Zubair Ashraf** · Originally published on [IBM SecurityIntelligence](https://web.archive.org/web/20140408042128/https://securityintelligence.com/how-to-cheat-your-mdm-compliance-without-a-password/), 2014 · archived copy, republished here. The full version with all formatting lives at [crlab.ca](https://crlab.ca/learn/security-research/how-to-cheat-your-mdm-compliance-without-a-password/)._

---

DID YOU KNOW? By 2016, 20% of enterprise BYOD programs will fail due to deployment of **mobile device management (MDM)** measures that are too restrictive. Gartner Recently, I came across a bug in Android’s Device Administrator code and questioning whether it can be classified as a security vulnerability and if/how it can be exploited. This resulted in the **CVE-2014-0900**. Below (as promised in my [OBAD analysis blog entry](https://crlab.ca/learn/security-research/diy-android-malware-analysis-taking-apart-obad-part-1/)), I will share the vulnerability in Android OS code and describe how it can be exploited to give a false sense of control and authority to an affected **MDM Android app**.
...
## **Disclaimer:** For your personal safety and job security, we strongly discourage trying this on your BYOD-approved device. Now the Details

The vulnerability exists in private void handlePackagesChanged(int userHandle) methods of [com.android.server.DevicePolicyManagerService](https://android.googlesource.com/platform/frameworks/base/+/android-4.3_r2/services/java/com/android/server/DevicePolicyManagerService.java) for versions below android-4.4.1_r1.
...
The first step is to get hold of the apk package for Lookout anti-virus (hint: install on your phone, and get the apk using adb pull) and to extract the AndroidManifest.xml file. If you are not familiar with how to do this, you can find some hints in my earlier post, [DIY: Android Malware Analysis – Taking Apart OBAD](https://crlab.ca/learn/security-research/diy-android-malware-analysis-taking-apart-obad-part-1/) . Look at the [AndroidManifest.xml](http://developer.android.com/guide/topics/manifest/manifest-intro.html) file, and take note of two things: The package name for the app and the name of the [DeviceAdministrator](http://developer.android.com/guide/topics/admin/device-admin.html) component. You can read up on the previously linked reference about DeviceAdministrator to learn how to create your own DeviceAdministrator.
...
_Lookout AV thinking that it has the feature requiring device administrator access enabled._

![Lookout AV thinking that it has the feature requiring device administrator access enabled.](https://crlab.ca/assets/security-research/how-to-cheat-your-mdm-compliance-without-a-password/img1.jpg)

_The real list of device administrators as seen from Settings -> Security -> Device Administrators – showing none is enabled._

![Security Device Admins List Showing Lookout Is Not an Admin](https://crlab.ca/assets/security-research/how-to-cheat-your-mdm-compliance-without-a-password/img2.jpg)

### One More Obstacle

If you tried the above steps on an MDM app, it likely did not work. The reason is that MDM apps typically also use other methods to check whether the device is compliant with the security policies. One common way is to check for password strength compliance using [isActivePasswordSufficient of DevicePolicyManager](http://developer.android.com/reference/android/app/admin/DevicePolicyManager.html#isActivePasswordSufficient%28%29). Taking a look at its [code](https://android.googlesource.com/platform/frameworks/base/+/android-4.3_r2/services/java/com/android/server/DevicePolicyManagerService.java#isActivePasswordSufficient), you will see:
...
![User (Laziness) Friendly](https://crlab.ca/assets/security-research/how-to-cheat-your-mdm-compliance-without-a-password/img3.png)

_A typical screenshot showing a device compliance message._

![A typical screenshot showing a device compliance message.](https://crlab.ca/assets/security-research/how-to-cheat-your-mdm-compliance-without-a-password/img4.jpg)
...
**Thanks for reading CR Labs.** If this was useful, subscribe and forward it to someone who'd like it. More: the full guide at [crlab.ca/learn/security-research/how-to-cheat-your-mdm-compliance-without-a-password/](https://crlab.ca/learn/security-research/how-to-cheat-your-mdm-compliance-without-a-password/), videos on [YouTube](https://crlab.ca/videos/), and updates on [X](https://x.com/zashraf1337) and [LinkedIn](https://www.linkedin.com/in/zubairashraf/).
