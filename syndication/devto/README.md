...
**How to publish each file:**
1. dev.to → **Create Post** → switch the editor to **Markdown** (the `...` menu → "Switch to Markdown editor") .
2. Paste the **entire** `.md` file, *including the `---` front-matter block* — dev.to reads `title`, `tags`,
   `canonical_url`, and `cover_image` straight from it.
3. The files ship with `published: false` (saves as a draft). Flip to `published: true` (or just hit Publish) when ready.
4. `canonical_url` is pre-set to the crlab.ca original, so dev.to shows "Originally published at crlab.ca" and
   won't compete with it in search.

Tags are pre-filled (dev.to allows max 4, lowercase). Cover image is the article's OG image on crlab.ca.
